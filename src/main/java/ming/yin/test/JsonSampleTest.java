...
public class JsonSampleTest {
	...
	private static TestType initThriftObject() {
	...
	private static void testThrift(int times) throws TException {
		TSerializer serializer = new TSerializer(new TBinaryProtocol.Factory());
		TDeserializer deserializer = new TDeserializer(new TBinaryProtocol.Factory());
		
		TestType testType = initThriftObject();
		...
		System.out.println("Thrift: " + completedIn.toString() + " = "  + DurationFormatUtils.formatDuration(completedIn, "HH:mm:ss:SS"));
		System.out.printf("Bytes: %d\n\n", serializer.serialize(testType).length);
	}

	private static void testJson(int times) {
		Map<String, Object> obj = JsonUtils.toMap(jsonRepresentation);
		
		long time = System.currentTimeMillis();
		for (int i = 0; i < times; i++) {
			String json = JsonUtils.toJSON(obj);
			Map<String, Object> objGenerated = JsonUtils.toMap(json);

			assert obj.equals(objGenerated);
		}
		Long completedIn = System.currentTimeMillis() - time;

		System.out.println("JSon: " + completedIn.toString() + " = " + DurationFormatUtils.formatDuration(completedIn, "HH:mm:ss:SS"));
		System.out.printf("Bytes: %d\n\n", jsonRepresentation.length());
	}
